            ...
            for noise_key in self.noise_keys:
                ...
                if len(x) >= self.n_data_per_epoch:
                    ...
                    _label = np.concatenate(
                        list(map(utils.moving_window_label(window_size=self.window_size + 1), lab)), axis=0)
                    
                    dataset = FSEGAN_Dataset(_noise, _clean, _label, transform=self.transform)
...
#fsegan_classifier = rnn_base_models.rnn_base_classifier(I=128, H=500).to(device)
